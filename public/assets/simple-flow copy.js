        ...
        stepEditorProvider: (step) => {
            ...
            if (step.type == "replayMessage") {
                ...
                dropdown.addEventListener('change', () => {
                    ...
                    pushOrUpdatePair(step.id, selectedType);
...
function runWorkflow() {
    ...
    for (const step of definition.sequence) {
        console.log(step);
        console.log(getValueByKey(step.id));
        if (step.type === 'replayMessage') {
...
// Function to check if a key already exists in the array
function isKeyUnique(key) {
    return keyValueArray.every(function(pair) {
        return pair.key !== key;
    });
}

// Function to push a key-value pair into the array or update the value if the key already exists
function pushOrUpdatePair(key, value) {
    var found = false;
    keyValueArray.forEach(function(pair) {
        if (pair.key === key) {
            pair.value = value; // Update the value if the key already exists
            found = true;
        }
    });
    if (!found) {
        keyValueArray.push({ key: key, value: value }); // Add a new key-value pair if it doesn't exist
    }
}
